...
import { ArrowLeft, Lock, Search, AlertCircle, User, Calendar, CheckCircle, Eye, Clock, FileText, Database, RefreshCw, UserCheck, UserX, FileSpreadsheet, MessageSquare, Bell } from 'lucide-react';
import { lookupUserBySecureId, isValidSecureId, getAccessLogs } from '../utils/adminService';
...
export default function AdminPanel() {
  ...
  const handleApplicationStatus = async (id: string, status: 'approved' | 'rejected', reason?: string) => {
    try {
  ...
  const handleSecureIdLookup = async (e: React.FormEvent) => {
    ...
    if (!isValidSecureId(secureId)) {
      setUserLookupError('Invalid secure ID format. IDs can only contain numbers, letters, commas, periods, underscores and dashes, with a maximum length of 64 characters.');
      return;
    }
    ...
    try {
      const { data, error } = await lookupUserBySecureId(secureId, lookupReason);
      ...
      setUserLookupError(err.message || 'An error occurred while looking up the secure ID');
    } finally {
    ...
    <div className="min-h-screen bg-slate-900 p-2 md:p-8">
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-4 md:mb-8">
          <Link to="/" className="inline-flex items-center text-slate-400 hover:text-white">
              ...
              <div className="bg-slate-700/30 backdrop-blur-sm rounded-xl p-3 md:p-6 mb-6">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    >
                      <RefreshCw className={`h-5 w-5 ${isLoadingApplications ? 'animate-spin' : ''}`} />
                    </button>
                  </div>
                </div>
                ...
                {isLoadingApplications ? (
                  ...
                  <div className="space-y-4">
                    {applications?.map((app) => (
                      <div key={app.id} className="bg-slate-800/50 backdrop-blur-sm rounded-xl p-4 md:p-6 border border-slate-700/50 hover:border-indigo-500/50 transition-all duration-300 group">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-4">
                          ...
                          <div>
                            <h4 className="text-sm font-medium text-slate-400 mb-2">YouTube Channels</h4>
                            <div className="space-y-2">
                              {app.youtube_links?.map((link, index) => link && (
                                <div key={index} className="flex items-center justify-between bg-slate-700/50 rounded-lg p-3 group-hover:bg-slate-700 transition-colors">
                                  <a
                                    href={link}
                                    target="_blank"
              ...
              {showRejectionModal && (
                <div className="fixed inset-0 bg-slate-900/90 backdrop-blur-sm flex items-center justify-center p-4 z-50">
                  <div className="bg-slate-800 rounded-xl p-6 max-w-md w-full border border-slate-700/50 shadow-xl">
                    <h3 className="text-xl font-semibold text-white mb-4">Provide Rejection Reason</h3>
                    <textarea
              ...
              <div className="bg-slate-700/30 backdrop-blur-sm rounded-xl p-3 md:p-6 mb-6">
                ...
                <form onSubmit={async (e) => {
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    />
                    <label htmlFor="sendToAll" className="text-sm font-medium text-slate-300">
                      Send to all users
                    </label>
                  </div>
